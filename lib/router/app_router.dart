    ...
    routes: [
        ...
        builder: (context, state, child) {
          ...
          return ChangeNotifierProvider(
            create: (_) => AccommodationProvider(
              service: AccommodationService(repository: repository),
            )..load(),
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          if (room == null) return const _MissingRecordScreen(title: 'Room Details');
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          if (payment == null) return const _MissingRecordScreen(title: 'Payment Details');
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          if (agreement == null) return const _MissingRecordScreen(title: 'Agreement Details');
          return AgreementDetailsScreen(agreement: agreement);
        },
      ),
      GoRoute(
        path: '/terms',
        builder: (context, state) => TermsScreen(terms: state.extra as List<TermItem>? ?? const []),
      ),
      GoRoute(
        path: '/coming-soon',
...
/// Shown when a detail route is opened directly (for example a browser reload)
/// without the record that normally travels with it.
class _MissingRecordScreen extends StatelessWidget {
  final String title;

  const _MissingRecordScreen({required this.title});
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                'This record is no longer loaded. Please open it again from the list.',
                textAlign: TextAlign.center,
                style: Theme.of(context).textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              FilledButton(onPressed: () => context.go('/rooms'), child: const Text('Back to Rooms')),
            ],
          ),
        ),
      ),
